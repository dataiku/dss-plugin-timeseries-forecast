...
class TrainingSession:
    ...
    def evaluate(self, evaluation_strategy):
        if evaluation_strategy == "split":
            self.train_ds = self.create_gluon_dataset(
                remove_length=self.prediction_length
            )  # remove last prediction_length time steps
        ...
        for model in self.models:
            if self.make_forecasts:
                (
                    ...
                    forecasts_df,
                    identifiers_columns,
                ) = model.evaluate(self.train_ds, self.test_ds, make_forecasts=True)
